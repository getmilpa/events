# Contributing to Milpa Events

Thanks for your interest in contributing! Milpa Events is the reference event dispatcher
of the Milpa framework — string-named events, dot-segment wildcard subscriptions
(`user.*`), priority ordering, listener error isolation, and a pluggable async (queue)
seam. It builds on `milpa/core`: it implements the
`Milpa\Interfaces\Event\MilpaEventDispatcherInterface` contract defined there, with no
product coupling.
...
- **Respect the tier boundary.** Milpa Events depends on `milpa/core` (it implements
  `MilpaEventDispatcherInterface`, defined there), never the reverse. Do not introduce
  a dependency on Doctrine, a concrete container, or any product/plugin code, and do
  not push dispatch concerns (queue wiring, channel-specific handling) down into the
  core.
...
The whole Milpa family (`milpa/core`, `milpa/http`, `milpa/tool-runtime`, `milpa/events`)
shares one coding standard, committed verbatim in every repo as `.php-cs-fixer.dist.php`
and enforced by CI. In short:
